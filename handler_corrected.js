    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                if (!isNumber(user.subscription_start_date)) user.subscription_start_date = null;
                if (!isNumber(user.subscription_end_date)) user.subscription_end_date = null;

                // Check for expired subscription
                if (user.subscription_active && user.subscription_end_date && user.subscription_end_date < Date.now()) {
                  user.subscription_active = false;
                  // Optionally notify the user about expiration here
                  // this.reply(m.sender, 'اشتراكك قد انتهى.', null);
                }
...
`.trim(), data.jid)
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (plugin.private && m.isGroup) { // Private Chat Only
                   ...
                   if (plugin.register == true && !_user.registered) { // Need register?
                    fail(\'unreg\', m, this)
                    continue
                }

                // Check Subscription
                const commandName = command; // command should be available in this scope
                const pluginFileName = name; // Assuming \'name\' (plugin filename) is available here
                const isMenuCommand = commandName === \'menu\';
                const isSubscriptionCommand = pluginFileName === \'add_subscription.js\'; // Match the plugin filename

                // Use plugin.subscription = true to mark commands needing subscription
                const requiresSubscription = plugin.subscription === true && !isMenuCommand && !isSubscriptionCommand;

                if (requiresSubscription) {
                    // Check if user has active subscription OR is owner/mod/premium
                    // Use _user which should be the user object from the database
                    if (!(_user.subscription_active && _user.subscription_end_date && _user.subscription_end_date > Date.now()) && !isOwner && !isMods && !isPrems) {
                        fail(\'nosubscription\', m, this); // Use the fail function
...
let msg = {
        ...
        nosubscription: 'تواصل مع المالك لي يضيف لك اشتراك'
        }[type]
